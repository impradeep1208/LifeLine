...
public class AdminController {
    ...
    @PostMapping("/fix-drivers")
    public ResponseEntity<?> fixDriverAccounts() {
        try {
            String passwordHash = "$2a$10$/rpORbek7gEeyRj0ptpYbeDKayjnEdzoKl3ndQ/308qE81xP1OEYS";
            
            // Delete conflicting driver accounts that might have wrong passwords
            jdbcTemplate.execute(
                "DELETE FROM users WHERE username IN ('driver1', 'driver2', 'driver3') AND id NOT LIKE 'demo-ambulance-%'"
            );
            
            // Now insert/update the correct driver accounts
            jdbcTemplate.execute(
                "INSERT INTO users (id, username, password, role, full_name, phone, email, is_active, created_at) " +
                "VALUES " +
                "('demo-ambulance-001', 'driver1', '" + passwordHash + "', 'AMBULANCE', 'Driver Suresh', '+91-9876543212', 'suresh@ems.com', true, CURRENT_TIMESTAMP), " +
            ...
            // Verify ambulances are linked to correct users
            jdbcTemplate.execute(
                "INSERT INTO ambulances (id, vehicle_number, operator_user_id, current_latitude, current_longitude, status, equipment, last_updated) " +
                "VALUES " +
                "('amb-demo-001', 'AP-31-EMR-1001', 'demo-ambulance-001', 17.6868, 83.2185, 'AVAILABLE', " +
            ...
            Map<String, Object> result = new HashMap<>();
            result.put("success", true);
            result.put("message", "Driver accounts fixed! All drivers (driver1, driver2, driver3) now have password: password123");
            result.put("drivers", "driver1, driver2, driver3");
    ...
    @GetMapping("/fix-drivers")
    public ResponseEntity<?> fixDriverAccountsGet() {
        return fixDriverAccounts();
    }
}
